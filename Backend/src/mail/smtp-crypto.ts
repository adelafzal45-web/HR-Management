...
/**
 * Symmetric encryption for the stored SMTP password.
 *
 * Why encryption and not hashing: unlike a user password, this one has to be
 * *recovered* — it is presented to the mail server on every connection. So the
 * requirement is reversible-with-a-key, not one-way.
 *
 ...
 * decrypting to garbage that then gets sent to an SMTP server as a login
 * attempt.
 *
 * What this protects against: a leaked database dump or a SQL-injection read
 * does not yield a usable mail credential. What it does not protect against:
 ...
 * SMTP_ENCRYPTION_KEY out of the database and out of version control.
 */

const logger = new Logger('SmtpCrypto');
...
/**
 * Derives the 32-byte key from SMTP_ENCRYPTION_KEY.
 *
 ...
 * existing convention in `auth.constants.ts` (JWT_SECRET does the same) so local
...
function encryptionKey(): Buffer {
  ...
  if (!configured) {
    if (!warnedAboutFallback) {
      logger.warn(
        'SMTP_ENCRYPTION_KEY is not set — falling back to the built-in development key. ' +
...
/** Encrypts a plaintext SMTP password for storage. */
...
/**
 * Decrypts a stored SMTP password.
 *
 * Throws rather than returning null on a malformed or tampered value: silently
 * treating it as "no password" would turn a key rotation mistake into an
 * unauthenticated connection attempt against the mail server, which is harder
 * to diagnose than an explicit failure.
 *
...
export function decrypt(stored: string): string {
  if (!isEncrypted(stored)) {
    throw new InternalServerErrorException(
      'Stored SMTP password is not in the expected encrypted format',
    );
  }
  ...
  if (!ivB64 || !tagB64 || !payloadB64) {
    throw new InternalServerErrorException(
      'Stored SMTP password is malformed and cannot be decrypted',
    );
  }
  ...
  try {
    ...
    // SMTP_ENCRYPTION_KEY has changed since it was written.
    throw new InternalServerErrorException(
      'Stored SMTP password could not be decrypted. It may have been written ' +
