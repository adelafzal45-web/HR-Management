...
export default function EmployeeDataGrid<T>({
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-gray-100 dark:bg-gray-900 dark:ring-gray-800">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <label className="relative block w-full sm:max-w-xs">
            ...
            <input
              ...
              className="w-full rounded-lg bg-gray-100 py-2.5 pl-9 pr-3 text-sm text-gray-800 outline-none placeholder:text-gray-400 focus:ring-2 focus:ring-brand/60 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500"
            />
          </label>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className={`relative flex min-h-10 items-center gap-1.5 rounded-lg border px-3 text-sm font-medium transition ${
                activeFilterCount > 0
                  ? "border-brand/60 bg-brand-light/40 text-brand-dark dark:bg-brand/10 dark:text-brand"
            ...
            <div className="relative" ref={columnsMenuRef}>
              <button
                ...
                className="flex min-h-10 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
              >
                <Columns3 size={15} /> Columns
              </button>
              {columnsMenuOpen && (
                <div className="absolute right-0 z-20 mt-2 w-56 rounded-xl border border-gray-100 bg-white p-2 shadow-lg dark:border-gray-800 dark:bg-gray-900">
                  <p className="px-2 pb-1.5 pt-1 text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Visible Columns</p>
                  {columns
                    .filter((c) => !c.locked)
                    .map((c) => (
                      <label key={c.key} className="flex cursor-pointer items-center gap-2.5 rounded-lg px-2 py-1.5 text-sm text-gray-700 hover:bg-gray-50 dark:text-gray-300 dark:hover:bg-gray-800">
                        <input
                          type="checkbox"
            ...
            <button
              type="button"
              onClick={() => onExport(selectedRows.length > 0 ? selectedRows : sortedRows)}
              className="flex min-h-10 items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3 text-sm font-medium text-gray-700 transition hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
            >
        ...
        {filterChips.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            {filterChips.map((chip) => (
              <span
                key={chip.key}
                className="flex items-center gap-1.5 rounded-full bg-brand-light/60 py-1 pl-3 pr-1.5 text-xs font-medium text-brand-dark dark:bg-brand/10 dark:text-brand"
              >
                {chip.label}
                <button
                  type="button"
                  ...
                  className="flex h-4 w-4 items-center justify-center rounded-full hover:bg-brand-dark/20 dark:hover:bg-brand/30"
                >
                  <X size={11} />
                </button>
              </span>
      ...
      {/* Bulk action bar */}
      {selectedIds.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 border-b border-brand/20 bg-brand-light/30 px-4 py-2.5 dark:border-brand/20 dark:bg-brand/10">
          <span className="text-sm font-medium text-brand-dark dark:text-brand">{selectedIds.size} selected</span>
          <div className="ml-auto flex flex-wrap items-center gap-2">
            {bulkActions.map((ba) => (
              <button
                ...
                className={`flex min-h-8 items-center gap-1.5 rounded-lg px-3 text-xs font-semibold transition ${
                  ba.tone === "danger"
                    ? "bg-white text-rose-600 ring-1 ring-rose-200 hover:bg-rose-50 dark:bg-gray-900 dark:text-rose-400 dark:ring-rose-500/30 dark:hover:bg-rose-500/10"
            ...
            <button
              ...
              className="text-xs font-medium text-brand-dark hover:underline dark:text-brand"
            >
              Clear
            </button>
          </div>
          ...
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-center gap-3">
              {[...Array(skeletonCols)].map((__, j) => (
                <div key={j} className="h-11 flex-1 animate-pulse rounded-xl bg-gray-100 dark:bg-gray-800" style={{ animationDelay: `${i * 40}ms` }} />
              ))}
            </div>
          ))}
        </div>
        ...
        <>
          ...
          <div className="hidden max-h-[65vh] overflow-auto sm:block">
            <table className="w-full text-left text-sm" style={{ tableLayout: "fixed" }}>
              ...
              <thead className="sticky top-0 z-10">
                <tr className="border-b border-gray-100 bg-gray-50/95 text-xs font-semibold uppercase tracking-wide text-gray-500 backdrop-blur dark:border-gray-800 dark:bg-gray-900/95 dark:text-gray-400">
              ...
              <tbody>
                {pageRows.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={id}
                      className={`border-b border-gray-50 transition-colors last:border-0 hover:bg-brand-light/20 dark:border-gray-800/60 dark:hover:bg-brand/5 ${
                        selected
                          ? "bg-brand-light/30 dark:bg-brand/10"
                          : i % 2 === 1
                    ...
                    >
                      ...
                      {visibleColumns.map((col) => (
                        <td
                          key={col.key}
                          className={`overflow-hidden px-4 py-3.5 align-middle text-gray-700 dark:text-gray-300 ${col.align ? ALIGN_CLASS[col.align] : ""}`}
                        >
                          {col.render(row)}
                        </td>
                      ))}
          ...
          <div className="divide-y divide-gray-50 sm:hidden dark:divide-gray-800">
            {pageRows.map((row) => {
              ...
              const selected = selectedIds.has(id);
              return (
                <div key={id} className={`p-4 ${selected ? "bg-brand-light/20 dark:bg-brand/10" : ""}`}>
                  <div className="mb-2 flex items-center gap-2.5">
                    <input
                      ...
                      className="h-4 w-4 rounded border-gray-300 text-brand focus:ring-brand/60 dark:border-gray-600 dark:bg-gray-800"
                    />
                    <span className="text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">Select</span>
                  </div>
                  <div className="space-y-1.5">
                    {visibleColumns.map((col) => (
                      <div key={col.key} className="flex items-start justify-between gap-3 text-sm">
                        <span className="shrink-0 text-xs font-medium uppercase tracking-wide text-gray-400 dark:text-gray-500">{col.label}</span>
                        <span className="min-w-0 text-right text-gray-700 dark:text-gray-300">{col.render(row)}</span>
                      </div>
                    ))}
                  </div>
                  {actions && <div className="mt-3 flex justify-end gap-2 border-t border-gray-50 pt-3 dark:border-gray-800">{actions(row)}</div>}
                </div>
              );
            })}
          </div>
      ...
      {/* Pagination */}
      {!loading && total > 0 && (
        <div className="flex flex-col items-center justify-between gap-3 border-t border-gray-100 p-4 text-sm text-gray-500 dark:border-gray-800 dark:text-gray-400 lg:flex-row">
          <div className="flex w-full flex-col items-center gap-3 xs:flex-row xs:justify-between lg:w-auto lg:justify-start lg:gap-5">
            <p>
              Showing <span className="font-medium text-gray-700 dark:text-gray-200">{rangeStart}–{rangeEnd}</span> of{" "}
            ...
            <label className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400">
              ...
              <select
                ...
                className="rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-brand/60 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-200"
              >
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div className="hidden items-center gap-1 sm:flex">
              {buildPageWindow(clampedPage, totalPages).map((p, i) =>
                p === "ellipsis" ? (
                  <span key={`e-${i}`} className="flex min-h-9 min-w-9 items-center justify-center text-sm text-gray-300 dark:text-gray-600">
                    …
                  </span>
                ) : (
                  <button
            ...
            <span className="min-w-[64px] text-center text-sm font-medium text-gray-700 dark:text-gray-200 sm:hidden">
