...
export default function ComingSoon() {
 ...
 // Falls back to looking the current URL up in the nav tree so a
 // bookmarked/typed-in link (no router state) still shows the right label
 // instead of the generic "This feature".
 const navMatch = findLeafByPath(location.pathname, user?.role, location.search);
 const featureLabel = state.label ?? navMatch?.label ?? "This feature";
 const activeKey = state.key ?? navMatch?.key ?? "";
