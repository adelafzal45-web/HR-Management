...
export default function RolesPage() {
 ...
 return { all: selected === perms.length && perms.length > 0, some: selected > 0 && selected < perms.length, selected };
 };
 ...
 const toggleAll = () => {
 if (readOnly) return;
 setForm((f) => ({ ...f, permissionIds: totalState.all ? [] : permissions.map((p) => p.permissionId) }));
 };
 ...
 <span className="text-[15px] font-medium text-gray-900">Permissions</span>
 {permissions.length > 0 && (
 <span className="text-xs text-gray-500">
 {selectedSet.size} of {permissions.length} selected
 </span>
 )}
 </div>
 ...
 {permissions.length > 0 && (
 <label
 className={`flex items-center gap-2.5 rounded-lg border border-gray-200 bg-white px-3 py-2.5 text-sm ${
 ...
 />
 <span className="font-semibold text-gray-900">Select All Permissions</span>
 </label>
 )}
 ...
 <div key={module}>
 <label
 className={`mb-2 flex items-center gap-2.5 text-xs font-semibold uppercase tracking-wide text-gray-500 ${
 ...
 <div className="grid grid-cols-1 gap-2 pl-6 xs:grid-cols-2">
 {perms.map((p) => (
 <label
 key={p.permissionId}
 className={`flex items-start gap-2.5 rounded-lg border px-3 py-2.5 text-sm transition ${
 selectedSet.has(p.permissionId) ? "border-brand bg-brand-light/60" : "border-gray-200 bg-white"
 } ${readOnly ? "" : "cursor-pointer hover:border-brand"}`}
 >
 <input
 type="checkbox"
 checked={selectedSet.has(p.permissionId)}
