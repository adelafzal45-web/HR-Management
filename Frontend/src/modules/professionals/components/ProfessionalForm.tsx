...
import { professionalsApi, type Professional, type ProfessionalCreatePayload, type ProfessionalPayload } from "@/modules/professionals/api/professionalApi";
import {
  departmentsApi,
  designationsApi,
  jobCategoriesApi,
...
export default function ProfessionalForm({ mode, professional, onCancel, onSaved }: Props) {
  const toast = useToast();
  const draftKey = mode === "create" ? "hrms.professionals.draft.create" : `hrms.professionals.draft.edit.${professional?.professionalId ?? ""}`;
  ...
  // Seed the form from the professional being edited (or leave it empty for
  ...
  useEffect(() => {
    if (mode === "edit" && professional) {
      setForm({
        ...
        password: "",
        ...splitAddress(professional.address),
        ...
        shiftId: professional.shiftId,
        managerId: professional.managerId,
        salary: String(professional.salary || ""),
        overtimeAllowed: professional.overtimeAllowed,
        bankName: "",
        accountNumber: "",
        ifsc: "",
        emergencyName: "",
  ...
  }, [mode, professional?.professionalId]);
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      professionalsApi.list({ pageSize: 1000 }).then((res) => setAllProfessionals(res.data)),
  ...
  const managerOptions = useMemo(
    () =>
      allProfessionals
        .filter((e) => e.professionalId !== professional?.professionalId)
        .map((e) => ({ value: e.professionalId, label: `${e.firstName} ${e.lastName}` })),
  ...
  const validate = (): Record<string, string> => {
    ...
    if (!form.professionalCode.trim()) e.professionalCode = "Professional code is required.";
  ...
  const buildPayload = (): ProfessionalCreatePayload | ProfessionalPayload => ({
    professionalCode: form.professionalCode.trim(),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "edit" && professional) {
        await professionalsApi.update(professional.professionalId, buildPayload() as ProfessionalPayload);
        toast.showSuccess("Professional updated.", `${form.firstName} ${form.lastName}'s record has been saved.`);
      } else {
        await professionalsApi.create(buildPayload() as ProfessionalCreatePayload);
        toast.showSuccess("Professional added.", `${form.firstName} ${form.lastName} has been added to your organization.`);
      }
      try {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[220px_1fr]">
        ...
        <div className="min-w-0 space-y-6">
          ...
          <SectionCard id="employment" title="Employment Details" icon={Briefcase} sectionRef={(el) => (sectionRefs.current.employment = el)}>
            <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-2">
              <FieldWithError label="Professional Code" placeholder="e.g. EMP-1011" value={form.professionalCode} onChange={(e) => update("professionalCode", e.target.value)} error={errors.professionalCode} required />
            ...
            <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <div className="flex min-h-[52px] items-center gap-2 rounded-lg bg-gray-100 px-4">
                  <StatusBadge status={professional?.status ?? "active"} />
                  <span className="text-xs text-gray-500">
                    {mode === "create" ? "New accounts start active." : "Change status from the professional list."}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="mx-auto flex max-w-[1600px] flex-col-reverse gap-2.5 xs:flex-row xs:items-center xs:justify-end">
          ...
          <div className="xs:w-56">
            <PrimaryButton type="submit" loading={saving}>
              {mode === "edit" ? "Save Changes" : "Create Professional"}
            </PrimaryButton>
          </div>
        </div>
      </div>
